...
export declare enum RegulatoryStatus {
    APPROVED = "APPROVED",
...
export declare enum CashDrawerStatus {
    OPEN = "OPEN",
...
export declare enum BusinessDateStatus {
    OPEN = "OPEN",
    CLOSING = "CLOSING",
    PENDING_APPROVAL = "PENDING_APPROVAL",
    LOCKED = "LOCKED",
...
export declare enum RecoveryBucket {
    CURRENT = "CURRENT",
    DPD_1_30 = "1-30",
    DPD_31_60 = "31-60",
    DPD_61_90 = "61-90",
...
export declare const BusinessRules: {
    readonly BR_001: "No customer without unique customer ID (SJF-CUS-XXXXXX)";
    readonly BR_002: "No approved transaction can be deleted; immutable audit trail";
    readonly BR_003: "Financial correction requires explicit reversal transaction";
    readonly BR_004: "Maker cannot approve own restricted transaction (maker_id != checker_id)";
    readonly BR_005: "Disabled products cannot accept new accounts or applications";
    readonly BR_006: "Cash drawer must reconcile daily before cashier shift closing";
    readonly BR_007: "Loan cannot be disbursed without full approval hierarchy";
    readonly BR_008: "Loan schedule must exist and balance before loan activation";
    readonly BR_009: "Business date must be closed daily across all active branches";
    readonly BR_010: "Closed business date cannot be edited without authorized reopening";
    readonly BR_011: "Audit logs are strictly append-only and immutable";
    readonly BR_012: "Every customer payment generates a unique transaction number (SJF-TXN-...)";
    readonly BR_013: "Every successful customer payment generates a digital receipt (SJF-RCP-...)";
    readonly BR_014: "Branch-level staff cannot access records from unauthorized branches";
    readonly BR_015: "Sensitive profile or financial updates require mandatory reason";
    readonly BR_016: "All double-entry ledger lines must balance: SUM(DR) === SUM(CR)";
    readonly BR_017: "Duplicate payment protection enforced via Idempotency-Key headers";
    readonly BR_018: "Physical or logical deletion of financial records is prohibited";
    readonly BR_019: "Product activation is governed by compliance & regulatory flags";
    readonly BR_020: "High-value transactions trigger maker-checker threshold approvals";
};
...
export interface IRedAlert {
    id: string;
    alertType: 'CASH_MISMATCH' | 'BANK_RECON_MISMATCH' | 'UNAPPROVED_HIGH_VALUE' | 'KYC_INCOMPLETE' | 'LOAN_HIGH_OVERDUE' | 'MATURITY_APPROACHING' | 'FAILED_LOGINS' | 'MANUAL_ADJUSTMENT' | 'TXN_REVERSAL' | 'DUPLICATE_SUSPECT';
